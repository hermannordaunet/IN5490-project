...
def initializer():
    ...
    parser = argparse.ArgumentParser(
        description="PyTorch Early-Exit Convolutional Neural Nets"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    if args.load_model is None:
        ...
        while os.path.exists(
            args.models_dir + "/model" + ".v" + str(args.start_epoch) + ".pt"
        ):
            args.start_epoch += 1
        if args.start_epoch > 1:
            model = torch.load(
                args.models_dir + "/model" + ".v" + str(args.start_epoch - 1) + ".pt"
            )
    ...
    print(
        "exit-block: flops={}, params={}, cost-rate={:.2f}".format(
