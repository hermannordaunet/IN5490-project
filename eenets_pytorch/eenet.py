...
from eenets_pytorch.flops_counter import get_model_complexity_info
from eenets_pytorch.resnet import ResNet, ResNet6n2
    ...
    * input_shape:        Input shape of the model according to dataset.
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.loss_func == "v3":
            self.exit_threshold = 1.0 / self.num_ee
        ...
        if is_6n2model:
            self.layers.append(
                nn.Sequential(
                    nn.Conv2d(
                        channel, 16, kernel_size=3, stride=1, padding=1, bias=False
                    ),
                    nn.BatchNorm2d(16),
